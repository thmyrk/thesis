class Janitor
  ...
  def self.generate_records
    ...
    subs = []
    SUB_COUNT.times do
      subs << Subscription.new(follower: users.sample, followed: users.sample)
    end

    likes = []
    LIKE_COUNT.times do
      likes << Like.new(post: posts.sample, user: users.sample)
    end

    comments = []
    COMMENT_COUNT.times do
      comments << Comment.new(post: posts.sample, user: users.sample, body: Faker::Hacker::say_something_smart)
    end
